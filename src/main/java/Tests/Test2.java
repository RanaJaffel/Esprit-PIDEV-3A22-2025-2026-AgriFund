...
public class Test2 {
    ...
    public static void main(String[] args) {
        try {
            ...
            Utils.MessagerieFileManager.initialiserRepertoires();
    ...
    private static void login() throws SQLException {
        ...
        if (utilisateurConnecte != null) {
            ...
            if (twoFAService.is2FAActive(utilisateurConnecte.getId())) {
                ...
                // Envoyer le code 2FA
                entities.Code2FA code = twoFAService.envoyerCode2FA(utilisateurConnecte.getId());
    ...
    private static void menuMessagerie() throws SQLException {
        while (true) {
            ...
            if (choix == 1) {
                ...
                Utils.EmojiManager.afficherListeEmojis();
                System.out.print("\nAppuyez sur Entrée pour continuer...");
                scanner.nextLine();
            } else if (choix == 0) {
                break;
    ...
    private static void afficherConversations() throws SQLException {
        List<entities.Conversation> conversations = messagerieService.listerConversations(utilisateurConnecte.getId());
        ...
        for (int i = 0; i < conversations.size(); i++) {
            entities.Conversation conv = conversations.get(i);
    ...
    private static void nouvelleConversation() throws SQLException {
        ...
        if (choix > 0 && choix <= autresUsers.size()) {
            ...
            entities.Conversation conv = messagerieService.creerOuRecupererConversation(
                    utilisateurConnecte.getId(),
                    destinataire.getId()
            );
     ...
     */
    private static void ouvrirConversation(entities.Conversation conv) throws SQLException {
        while (true) {
            ...
            List<entities.Message> messages = messagerieService.listerMessages(conv.getId(), utilisateurConnecte.getId());
     ...
     * AFFICHER UN MESSAGE
     */
    private static void afficherMessage(entities.Message msg) throws SQLException {
        ...
        if (msg.isaPieceJointe() && msg.getNbPiecesJointes() > 0) {
            try {
                List<entities.PieceJointe> pieces = messagerieService.listerPiecesJointes(msg.getId());

                if (!pieces.isEmpty()) {
                    System.out.println("   📎 Pièce(s) jointe(s) (" + pieces.size() + "):");
                    for (entities.PieceJointe pj : pieces) {
    ...
    private static void envoyerMessage(int conversationId) throws SQLException {
        ...
        String contenuAvecEmojis = Utils.EmojiManager.convertirEmojis(contenu);

        entities.Message msg = messagerieService.envoyerMessage(
                conversationId,
                utilisateurConnecte.getId(),
                contenuAvecEmojis
        );
     ...
     * MODIFIER UN MESSAGE
     */
    private static void modifierMessage(List<entities.Message> messages) throws SQLException {
        // Filtrer seulement mes messages
        List<entities.Message> mesMessages = new ArrayList<>();
        for (entities.Message msg : messages) {
        ...
        for (int i = 0; i < mesMessages.size(); i++) {
            entities.Message msg = mesMessages.get(i);
        ...
        if (choix > 0 && choix <= mesMessages.size()) {
            entities.Message msg = mesMessages.get(choix - 1);
            ...
            if (!nouveauContenu.trim().isEmpty()) {
                String nouveauAvecEmojis = Utils.EmojiManager.convertirEmojis(nouveauContenu);
     ...
     */
    private static void supprimerMessage(List<entities.Message> messages) throws SQLException {
        // Filtrer seulement mes messages
        List<entities.Message> mesMessages = new ArrayList<>();
        for (entities.Message msg : messages) {
            if (msg.getExpediteurId() == utilisateurConnecte.getId()) {
    ...
    private static void menuSecurite() throws SQLException {
        while (true) {
            ...
            entities.Parametres2FA params = twoFAService.getParametres(utilisateurConnecte.getId());
    ...
    private static void activer2FA() throws SQLException {
        ...
        if (params == null) {
            params = new entities.Parametres2FA(utilisateurConnecte.getId());
        }
    ...
    private static void afficherHistoriqueConnexions() throws SQLException {
        ...
        try {
            pst = Utils.DatabaseConnection.getConnection().prepareStatement(sql);
    ...
    private static void envoyerFichier(int conversationId, String typeFichier) throws SQLException {
        ...
        System.out.println("📦 Taille du fichier: " + Utils.FileManager.formaterTaille(taille));
        ...
        try {
            ...
            entities.Message msg = messagerieService.envoyerMessage(
                    conversationId,
                    utilisateurConnecte.getId(),
                    Utils.EmojiManager.convertirEmojis(message)
            );
            ...
            switch (typeFichier) {
                case "image":
                    cheminDestination = Utils.MessagerieFileManager.uploadImage(cheminSource, msg.getId());
                    break;
                case "document":
                    cheminDestination = Utils.MessagerieFileManager.uploadDocument(cheminSource, msg.getId());
                    break;
                case "audio":
                    cheminDestination = Utils.MessagerieFileManager.uploadAudio(cheminSource, msg.getId());
                    break;
            }
            ...
            entities.PieceJointe pj = messagerieService.ajouterPieceJointe(
                    msg.getId(),
                    cheminDestination,
                    source.getName()
            );
