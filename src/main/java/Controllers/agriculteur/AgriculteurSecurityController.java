...
public class AgriculteurSecurityController {
    ...
    private void loadSecurityStatus() throws SQLException {
        ...
        if (is2FAActive) {
            ...
            toggle2FAButton.getStyleClass().removeAll("btn-primary");
            ...
            Label warning = new Label("Activez l'authentification à deux facteurs pour sécuriser votre compte.");
    ...
    public void toggle2FA() {
        try {
            if (is2FAActive) {
                ...
                loadSecurityStatus();
                showAlert("2FA activée! Les codes seront envoyés par email.", Alert.AlertType.INFORMATION);
            }
        } catch (SQLException e) {
            showAlert("Erreur: " + e.getMessage(), Alert.AlertType.ERROR);
        }
                ...
                result.ifPresent(codeEntre -> {
                    try {
                        if (twoFAService.verifierCode(currentUserId, codeEntre)) {
                            showAlert("✓ Code vérifié! La 2FA fonctionne correctement.", Alert.AlertType.INFORMATION);
                        } else {
