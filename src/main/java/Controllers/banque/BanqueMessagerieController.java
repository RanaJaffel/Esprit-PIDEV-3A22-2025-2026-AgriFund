...
import Utils.EmojiManager;
import entities.Conversation;
import entities.Message;
import entities.Utilisateur;
import entities.PieceJointe;
...
public class BanqueMessagerieController {
    ...
    private HBox createConversationItem(Conversation conv) throws SQLException {
        ...
        typeLabel.setStyle("-fx-text-fill: #089647; -fx-font-size: 11px;");
    ...
    private void openConversation(Conversation conv) throws SQLException {
        ...
        if (correspondant != null) {
            ...
            try {
                String type = authService.getTypeUtilisateur(correspondantId);
                if ("AGRICULTEUR".equals(type)) {
                    chatUserType.setText("🌾 Agriculteur");
                } else if ("ADMIN".equals(type)) {
    ...
    private HBox createMessageBubble(Message msg) throws SQLException {
        ...
        if (isMine) {
            bubble.setStyle(bubble.getStyle() + "-fx-background-color: #E1B323;");
        ...
        if (msg.isaPieceJointe()) {
            addAttachments(bubble, msg, true);
        }
    ...
    private ContextMenu createMessageContextMenu(Message msg) {
        ...
        copyItem.setOnAction(e -> {
            ...
            content.putString(msg.getContenu());
    ...
    private void showDeleteMessageConfirmation(Message message) {
        ...
        // Style du bouton supprimer en rouge
    ...
    private void addAttachments(VBox bubble, Message msg, boolean isMine) {
        try {
            ...
            for (PieceJointe pj : pieces) {
                ...
                fileSize.setStyle("-fx-font-size: 10px; -fx-text-fill: rgba(0,0,0,0.5);");
    ...
    public void sendMessage() {
        ...
        try {
            ...
            Message msg = messagerieService.envoyerMessage(
                    currentConversation.getId(), currentUserId, contentWithEmojis);
    ...
    public void attachFile() {
        if (currentConversation == null) {
            showAlert("Sélectionnez une conversation", Alert.AlertType.WARNING);
            return;
        }
        ...
        if (file != null) {
            try {
                Message msg = messagerieService.envoyerMessage(
                        currentConversation.getId(), currentUserId, "📎 " + file.getName());

                if (msg != null) {
                    String type = Utils.MessagerieFileManager.determinerTypeFichier(file.getName());
                    String storedPath = null;

                    switch (type) {
                        case "image":
                            storedPath = Utils.MessagerieFileManager.uploadImage(file.getAbsolutePath(), msg.getId());
                            break;
                        default:
                            storedPath = Utils.MessagerieFileManager.uploadDocument(file.getAbsolutePath(), msg.getId());
                    }

                    if (storedPath != null) {
                        messagerieService.ajouterPieceJointe(msg.getId(), storedPath, file.getName());
                    }
    ...
    public void showNewConversationDialog() {
        try {
            ...
            List<String> choices = users.stream()
                    ...
                    .filter(u -> {
                        try {
                            ...
                            return "AGRICULTEUR".equals(type) || "ADMIN".equals(type);
                        } catch (Exception e) {
                            return false;
                        }
                    })
            ...
            if (choices.isEmpty()) {
                showAlert("Aucun contact disponible", Alert.AlertType.INFORMATION);
                return;
            }

            ChoiceDialog<String> dialog = new ChoiceDialog<>(choices.get(0), choices);
            dialog.setTitle("Nouvelle conversation");
            dialog.setHeaderText("Contacter un agriculteur ou administrateur");
