...
public class AgriculteurCapteurController {
    ...
    private void ajouterCapteur() {
        ...
        // Vérifier que l'agriculteur a au moins un projet
        if (cbProjet.getItems().isEmpty()) {
            afficherErreur("Pas de projet", "Vous devez d'abord créer un projet agricole avant d'ajouter un capteur.");
            return;
        }

        if (cbProjet.getValue() == null) {
            afficherErreur("Projet requis", "Veuillez sélectionner un projet pour ce capteur.");
            return;
        }
        ...
        try {
            ...
            // Vérifier la localisation via API
